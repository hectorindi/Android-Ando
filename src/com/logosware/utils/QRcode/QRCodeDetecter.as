...
*/
		...
		public function detect():Array {
			...
			LabelingObj.Labeling( bd, 10, 0xFF88FFFE, true ); // ???????
		...
		private function _clipCodes( bd:BitmapData, codes:Array):Array {
			...
			for ( var i:int = 0; i < codes.length; i++ ) {
				...
				var theta:Number = -Math.atan2( vector12.y, vector12.x ); // ???????
				
				var matrix:Matrix = new Matrix();
				var d:Number = (0.5 * marker1.width) / (Math.abs(Math.cos( theta )) + Math.abs(Math.sin( theta ) ) ); // ?????????????
				...
				var len:Number = ( vector12.length + 2 * d ); // QR?????????
		...
		private function _searchBorders(bmp:BitmapData, rectArray:Array, colorArray:Array):Array {
			...
			for ( var i:int = 0; i < rectArray.length; i++ ) {
				var count:int = 0;
				var target:Number = 0;
				var tempRect:Rectangle = rectArray[i];// ??
